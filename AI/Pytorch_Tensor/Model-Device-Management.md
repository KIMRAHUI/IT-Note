...
#### 개요
[[Device-Management-CPU-GPU]] 노트에서 **텐서**의 연산 장치를 어떻게 지정하는지 배웠습니다. 이 노트에서는 **모델(nn.Module)** 자체의 연산 장치를 어떻게 다루는지 정리합니다.
...
- `.to("cuda")`와 `.cuda()`는 결과가 동일합니다. 다만 `.to(device)` 방식이 device 변수 하나로 CPU/GPU를 자동 전환할 수 있어 더 권장됩니다. (자세한 이유는 [[Device-Management-CPU-GPU]] 참고)
...
모델 파라미터 텐서는 GPU에 있는 반면, 모델에 입력된 텐서는 CPU에 있어서 텐서 간 연산이 불가능하다는 에러가 발생합니다. (에러 메시지는 [[Device-Management-CPU-GPU]]에서 다룬 디바이스 불일치 에러와 동일한 원리입니다.)
...
자세한 학습 루프 패턴은 [[Training-Loop-and-Optimizer]] 노트를 참고하세요.
